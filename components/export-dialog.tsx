...
export function generateMermaid(doc: Doc): string {
  ...
  doc.lanes.forEach((lane) => {
    ...
    lines.push(`  subgraph ${safeId(lane.id)}["${lane.actor}"]`)
  ...
  doc.connections.forEach((c) => {
    const dashed = c.type === "data" || c.type === "uses" || !!MECHANISM_META[c.mechanism].dash
    const arrow = dashed ? "-.->" : "-->"
    const parts = [c.label, c.type === "yes" && !c.label ? "Yes" : c.type === "no" && !c.label ? "No" : undefined]
    if (isHandoff(doc, c)) parts.push(`[${MECHANISM_META[c.mechanism].label}${c.payload ? `: ${c.payload}` : ""}]`)
...
export const generateJSON = (doc: Doc, name: string) => JSON.stringify({ version: 4, name, ...doc }, null, 2)
...
const LIGHT: Palette = { bg: "#fafafa", grid: "#e5e5e5", card: "#ffffff", text: "#171717", muted: "#737373", laneLine: "#d4d4d4" }
const DARK: Palette = { bg: "#0a0a0a", grid: "#262626", card: "#171717", text: "#fafafa", muted: "#a3a3a3", laneLine: "#333333" }
...
export function generateSVG(doc: Doc, dark: boolean): string {
  ...
  const font = `${ff}="system-ui, sans-serif"`
  ...
  Object.entries(EDGE_TYPE_META).forEach(([type, meta]) => {
    out.push(`<marker id="arrow-${type}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="${meta.color}"/></marker>`)
  })
  out.push(`</defs>`)
  ...
  boxes.forEach((l) => {
    ...
    out.push(`<rect x="0" y="${l.top}" width="${laneW}" height="${l.height}" fill="${color}" fill-opacity="0.05" stroke="${p.laneLine}" ${sw}="1"/>`)
    out.push(`<rect x="0" y="${l.top}" width="${LANE_HEADER_W}" height="${l.height}" fill="${p.card}" stroke="${p.laneLine}" ${sw}="1"/>`)
    out.push(`<circle cx="16" cy="${l.top + l.height / 2}" r="4" fill="${color}"/>`)
    out.push(`<text x="26" y="${l.top + l.height / 2 + 4}" fill="${p.text}" ${fs}="12" ${fw}="600" ${font}>${escapeXml(l.actor)}</text>`)
  })
  ...
  doc.connections.forEach((c) => {
    ...
    const mech = MECHANISM_META[c.mechanism]
    out.push(`<path d="M ${x1} ${y1} C ${x1 + bend} ${y1}, ${x2 - bend} ${y2}, ${x2} ${y2}" fill="none" stroke="${meta.color}" ${sw}="2" ${mech.dash ? `${sda}="${mech.dash}"` : ""} marker-end="url(#arrow-${c.type})"/>`)
    const handoff = isHandoff(doc, c)
    const text = c.label || (handoff ? mech.short : "")
    if (text) {
      const mx = (x1 + x2) / 2, my = (y1 + y2) / 2
      const w = text.length * 6 + 16
      out.push(`<rect x="${mx - w / 2}" y="${my - 9}" width="${w}" height="18" rx="9" fill="${p.card}" stroke="${handoff ? mech.color : meta.color}" ${sw}="1"/>`)
  ...
  doc.nodes.forEach((n) => {
    ...
    out.push(`<rect x="${n.x}" y="${n.y}" width="${NODE_W}" height="${NODE_H}" rx="10" fill="${p.card}" stroke="${color}" ${sw}="2" ${n.type === "decision" ? `${sda}="6 4"` : ""}/>`)
...
export function ExportDialog({ doc, workflowName }: ExportDialogProps) {
  const [copied, setCopied] = useState<string | null>(null)
  const [open, setOpen] = useState(false)
  const [busy, setBusy] = useState(false)
  const { resolvedTheme } = useTheme()
  const dark = resolvedTheme === "dark"
  const slug = workflowName.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "workflow"

  const mermaid = generateMermaid(doc)
  const json = generateJSON(doc, workflowName)
  const svg = open ? generateSVG(doc, dark) : ""
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="flex max-h-[80vh] max-w-2xl flex-col overflow-hidden">
        <DialogHeader>
          <DialogTitle>Export {workflowName}</DialogTitle>
        </DialogHeader>

        <Tabs defaultValue="image" className="flex flex-1 flex-col overflow-hidden">
          ...
          <TabsContent value="image" className="mt-4 flex-1 space-y-4 overflow-auto">
            <div className="flex gap-2">
              <Button className="flex-1" onClick={exportPNG} disabled={busy}>
                <Download className="mr-2 h-4 w-4" />
                {busy ? "Exporting…" : "Download PNG"}
              </Button>
              <Button className="flex-1" variant="secondary" onClick={() => download(svg, `${slug}.svg`, "image/svg+xml")}>
                <Download className="mr-2 h-4 w-4" />
                Download SVG
              </Button>
            </div>
          ...
          <TabsContent value="code" className="mt-4 flex-1 space-y-4 overflow-auto">
            <CodeBlock title="Mermaid" code={mermaid} copied={copied === "mermaid"} onCopy={() => copy(mermaid, "mermaid")} onDownload={() => download(mermaid, `${slug}.mmd`)} />
            <CodeBlock title="JSON" code={json} copied={copied === "json"} onCopy={() => copy(json, "json")} onDownload={() => download(json, `${slug}.json`, "application/json")} />
          </TabsContent>
        </Tabs>
      </DialogContent>
    </Dialog>
